<header class="z-40" :class="{ 'dark': $store.app.semidark && $store.app.menu === 'collapsible-vertical' }">
    <div class="shadow-sm">
                    ...
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                        ...
                        <template x-for="notification in notifications">
                            <li class=" dark:text-white-light/90 ">
                                <a :href="notification.url" @click.prevent="markAsRead(notification.id, notification.url)">
                                    <div class="flex items-center py-2 group" @click.self="toggle">
                            ...
                            <li>
                                <div class="p-4">
                                    <form action="{{ route('notifications.markAllAsRead') }}" method="POST">
                                        @csrf
                                        <button type="submit" class="btn btn-primary block w-full btn-small">Marcar todo como leído</button>
                                    </form>
                                </div>
                            </li>
                        </template>
                ...
                <div class="dropdown flex-shrink-0" x-data="dropdown" @click.outside="open = false">
                    <a href="javascript:;" class="relative group" @click="toggle()">
                        <span><img class="w-9 h-9 rounded-full object-cover saturate-50 group-hover:saturate-100"
                                src="{{ Auth::user()->imagen }}" alt="image" /></span>
                    </a>
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                        ...
                        <li>
                            <a href="{{ route('apps.users.index')}}" class="dark:hover:text-white" @click="toggle">
                        ...
                        <li class="border-t border-white-light dark:border-white-light/10">
                            <a href="{{ route('logout') }}" class=" text-danger !py-3" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
        ...
        Alpine.data("header", () => ({
            ...
            notifications: [
                @foreach ($unreadNotifications as $notification)
                    {
                        id: '{{ $notification->id }}',
                        profile: '{{ $notification->data["image"] }}',
                        message: '<strong class="text-sm mr-1">{{ $notification->data["name"] }}</strong>{{ $notification->data["text"] }}',
                        time: '{{ $notification->created_at->diffForHumans() }}',
                        url:  '{{ route($notification->data["route"]) }}',
                    },
                @endforeach
            ],
            ...
            markAsRead(notificationId, url) {
                fetch(`/notification/mark-as-read/${notificationId}`, {
                    ...
                    headers: {
                        ...
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'), // Asegúrate de tener un meta tag para el CSRF token
                    },
                    body: JSON.stringify({ id: notificationId })
                }).then(response => {
                    if (response.ok) {
                        window.location.href = url; // Redirige después de marcar como leída
                    }
