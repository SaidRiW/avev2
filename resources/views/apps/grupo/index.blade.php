<x-layout.default>
    ...
    @include('apps.grupo.create')
    @include('apps.grupo.edit')
    @include('apps.grupo.delete')
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("striped", () => ({
                init() {
                    const tableOptions = {
                        data: {
                            headings: ["#", "Nomenclatura", "Carrera", "Fecha de creación", "Fecha de modificación", "Acciones"],
                            data: [
                                @php
                                    $cont = 1 ;  
                                @endphp       
                    ...
                    function showModalEdit(id, nombre, carreraNombre) {
                        const modalEdit = document.getElementById('modalEdit');
                        const editForm = document.getElementById('editForm');
                        editForm.action = '/apps/grupo/' + id;
                        editForm.querySelector('input[name="nombre"]').value = nombre;

                        // Obtener el ID de la carrera correspondiente al nombre
                        const select = editForm.querySelector('select[name="carrera"]');
                        for (let option of select.options) {
                            if (option.textContent.trim() === carreraNombre) {
                    ...
                    function showModalDelete(id, nombre) {
                        ...
                        deleteForm.action = '/apps/grupo/' + id;
                        deleteForm.querySelector('input[name="nombre"]').value = nombre;
                    ...
                    editButtons.forEach(button => {
                        button.addEventListener('click', function () {
                            const id = this.dataset.id; // Obtener el ID del grupo
                            const nombre = this.parentNode.parentNode.querySelector('td:nth-child(2)').textContent.trim(); // Obtener el nombre del grupo
                            const carreraNombre = this.parentNode.parentNode.querySelector('td:nth-child(3)').textContent.trim(); // Obtener el nombre de la carrera
                            showModalEdit(id, nombre, carreraNombre);
                        });
                    });
